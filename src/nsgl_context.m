...
static GLFWglproc getProcAddressNSGL(const char *procname) {
  ...
  GLFWglproc symbol =
      CFBundleGetFunctionPointerForName(_glfw.nsgl.framework, symbolName);
  ...
  if (ctxconfig->major > 2) {
    if (ctxconfig->major == 3 && ctxconfig->minor < 2) {
      _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                      "NSGL: The targeted version of macOS does not support "
                      "OpenGL 3.0 or 3.1 but may support 3.2 and above");
      return GLFW_FALSE;
    }
  }
...
#define SET_ATTRIB(a, v)                                                       \
  ...
  if (fbconfig->redBits != GLFW_DONT_CARE &&
    ...
    if (fbconfig->redBits == 16 && fbconfig->greenBits == 16 &&
